...
@interface FXYImageManager()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation FXYImageManager

CGSize AssetGridThumbnailSize;//缩略图大小
CGFloat FXYScreenWidth;
CGFloat FXYScreenScale;

static FXYImageManager *manager;
...
- (void)setPhotoWidth:(CGFloat)photoWidth {
    ...
    FXYScreenWidth = photoWidth / 2;
}

- (void)setColumnNumber:(NSInteger)columnNumber {
    [self configFXYScreenWidth];
    
    _columnNumber = columnNumber;
    CGFloat margin = 4;
    CGFloat itemWH = (FXYScreenWidth - 2 * margin - 4) / columnNumber - margin;
    AssetGridThumbnailSize = CGSizeMake(itemWH * FXYScreenScale, itemWH * FXYScreenScale);
}

- (void)configFXYScreenWidth {
    FXYScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    FXYScreenScale = 2.0;
    if (FXYScreenWidth > 700) {
        FXYScreenScale = 1.5;
    }
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(FXYAlbumModel *model))completion {
    __block FXYAlbumModel *model;
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<FXYAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<FXYAssetModel *> *))completion {
    FXYImagePickerConfig *config = [FXYImagePickerConfig sharedInstance];
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<FXYAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        FXYAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(FXYAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (FXYAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    FXYAssetModel *model;
    FXYAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == FXYAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == FXYAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == FXYAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == FXYAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [FXYAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (FXYAssetModelMediaType)getAssetType:(PHAsset *)asset {
    FXYAssetModelMediaType type = FXYAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = FXYAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = FXYAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = FXYAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = FXYAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != FXYAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = FXYScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < FXYScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = AssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * FXYScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(FXYAlbumModel *)model completion:(void (^)(UIImage *))completion {
    id asset = [model.result lastObject];
    if (!self.sortAscendingByModificationDate) {
        asset = [model.result firstObject];
    }
    return [[FXYImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([FXYImagePickerConfig sharedInstance].needFixComposition) {
...
- (FXYAlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    FXYAlbumModel *model = [[FXYAlbumModel alloc] init];
...
- (FXYAssetModel *)createModelWithAsset:(PHAsset *)asset {
    FXYAssetModelMediaType type = [[FXYImageManager manager] getAssetType:asset];
    NSString *timeLength = type == FXYAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[FXYImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    FXYAssetModel *model = [FXYAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
