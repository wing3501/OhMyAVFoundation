...
#import "FXYImageCropManager.h"
@interface FXYPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation FXYPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [FXYImageManager manager].shouldFixOrientation = YES;
    FXYImagePickerController *_tzImagePickerVc = (FXYImagePickerController *)self.navigationController;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_currentIndex) {
        [_collectionView setContentOffset:CGPointMake((self.view.fxy_width + 20) * self.currentIndex, 0) animated:NO];
    }
...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UIImage fxy_imageNamedFromMyBundle:@"navi_back"] forState:UIControlStateNormal];
...
- (void)configCollectionView {
    ...
    _collectionView.contentSize = CGSizeMake(self.models.count * (self.view.fxy_width + 20), 0);
    [self.view addSubview:_collectionView];
    [_collectionView registerClass:[FXYPhotoPreviewCell class] forCellWithReuseIdentifier:@"FXYPhotoPreviewCell"];
    [_collectionView registerClass:[FXYVideoPreviewCell class] forCellWithReuseIdentifier:@"FXYVideoPreviewCell"];
    [_collectionView registerClass:[FXYGifPreviewCell class] forCellWithReuseIdentifier:@"FXYGifPreviewCell"];
}
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop && _tzImagePickerVc.allowPickingImage) {
        ...
        [FXYImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat naviBarHeight = statusBarHeight + _tzImagePickerVc.navigationBar.fxy_height;
    _naviBar.frame = CGRectMake(0, 0, self.view.fxy_width, naviBarHeight);
    _backButton.frame = CGRectMake(10, 10 + statusBarHeightInterval, 44, 44);
    _selectButton.frame = CGRectMake(self.view.fxy_width - 56, 10 + statusBarHeightInterval, 44, 44);
    _indexLabel.frame = _selectButton.frame;
    
    _layout.itemSize = CGSizeMake(self.view.fxy_width + 20, self.view.fxy_height);
    _layout.minimumInteritemSpacing = 0;
    _layout.minimumLineSpacing = 0;
    _collectionView.frame = CGRectMake(-10, 0, self.view.fxy_width + 20, self.view.fxy_height);
    ...
    CGFloat toolBarTop = self.view.fxy_height - toolBarHeight;
    _toolBar.frame = CGRectMake(0, toolBarTop, self.view.fxy_width, toolBarHeight);
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
    ...
    _doneButton.frame = CGRectMake(self.view.fxy_width - _doneButton.fxy_width - 12, 0, _doneButton.fxy_width, 44);
    _numberImageView.frame = CGRectMake(_doneButton.fxy_left - 24 - 5, 10, 24, 24);
...
- (void)select:(UIButton *)selectButton {
    FXYImagePickerController *_tzImagePickerVc = (FXYImagePickerController *)self.navigationController;
    FXYAssetModel *model = _models[self.currentIndex];
    if (!selectButton.isSelected) {
        // 1. select:check if over the maxImagesCount / 选择照片,检查是否超过了最大个数的限制
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:[NSBundle fxy_localizedStringForKey:@"Select a maximum of %zd photos"], _tzImagePickerVc.maxImagesCount];
            ...
            if (model.type == FXYAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                [_tzImagePickerVc showAlertWithTitle:[NSBundle fxy_localizedStringForKey:@"Select the video when in multi state, we will handle the video as a photo"]];
            }
        }
    } else {
        ...
        for (FXYAssetModel *model_item in selectedModels) {
            if ([model.asset.localIdentifier isEqualToString:model_item.asset.localIdentifier]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    FXYAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:FXYOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:FXYOscillatoryAnimationToSmaller];
}

- (void)backButtonClick {
    if (self.navigationController.childViewControllers.count < 2) {
        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
        if ([self.navigationController isKindOfClass: [FXYImagePickerController class]]) {
...
- (void)doneButtonClick {
    ...
    if (_progress > 0 && _progress < 1 && (_selectButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
        _alertView = [_tzImagePickerVc showAlertWithTitle:[NSBundle fxy_localizedStringForKey:@"Synchronizing photos from iCloud"]];
        return;
    }
    ...
    FXYPhotoPreviewCell *cell = (FXYPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
    if (_tzImagePickerVc.allowCrop && [cell isKindOfClass:[FXYPhotoPreviewCell class]]) { // 裁剪状态
        _doneButton.enabled = NO;
        [_tzImagePickerVc showProgressHUD];
        UIImage *cropedImage = [FXYImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [FXYImageCropManager circularClipImage:cropedImage];
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    offSetWidth = offSetWidth +  ((self.view.fxy_width + 20) * 0.5);
    
    NSInteger currentIndex = offSetWidth / (self.view.fxy_width + 20);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FXYAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == FXYAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FXYVideoPreviewCell" forIndexPath:indexPath];
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == FXYAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FXYGifPreviewCell" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FXYPhotoPreviewCell" forIndexPath:indexPath];
        FXYPhotoPreviewCell *photoPreviewCell = (FXYPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[FXYPhotoPreviewCell class]]) {
        [(FXYPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[FXYPhotoPreviewCell class]]) {
        [(FXYPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[FXYVideoPreviewCell class]]) {
        FXYVideoPreviewCell *videoCell = (FXYVideoPreviewCell *)cell;
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == FXYAssetModelMediaTypeVideo) {
...
- (void)showPhotoBytes {
    [[FXYImageManager manager] getPhotosBytesWithArray:@[_models[self.currentIndex]] completion:^(NSString *totalBytes) {
        self->_originalPhotoLabel.text = [NSString stringWithFormat:@"(%@)",totalBytes];
    }];
}

- (NSInteger)currentIndex {
    return [FXYCommonTools fxy_isRightToLeftLayout] ? self.models.count - _currentIndex - 1 : _currentIndex;
}
